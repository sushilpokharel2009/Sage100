...
//		Get the list of tokenized Account Numbers
		...
		List<ReconDTO> reconDTOTokenizedList = tokenizationAdapter.populatePaymentAccountsWithTokenizedAccountNumbers(reconDTOList);
...
//		Update the ReconDTO with tokenized values
		...
		if (null != reconDTOTokenizedList) {
			...
			reconDTOTokenizedList.forEach(reconDTO -> {
				...
				paymentAccount.setAbaNum(reconDTO.getAbaNum());
