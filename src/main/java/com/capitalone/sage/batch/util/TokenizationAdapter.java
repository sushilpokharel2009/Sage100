...
//import com.capitalone.chassis.engine.model.exception.SystemNotAvilableException;
...
public class TokenizationAdapter {
	...
	public List<ReconDTO> populatePaymentAccountsWithTokenizedAccountNumbers(List<ReconDTO> paymentAccounts){
		...
		for(ReconDTO paymentAccountEntity : paymentAccounts) {
			try {
				if(StringUtils.isNumeric(paymentAccountEntity.getBankAccountNumber()))
					banTokensList.add(turingElementBuilder.createBanWithAbaForTokenize(paymentAccountEntity.getBankAccountNumber(), StringUtils.leftPad(paymentAccountEntity.getAbaNumber(), minimumTuringLength, paddingCharacter), paymentAccountEntity.getPmtAcctUnqId().toString()));
			}catch(Exception e) {
...
//		Actual BAN Tokenization call
		...
		tokenizedBans.forEach(turingElement -> {
			if(StringUtils.isNumeric(turingElement.getValue()) || turingElement.hasError()) {
				logger.warn("Bank Account Number Tokenization Failed. Tokenized Account Number is Bllank for Payment Reference Id:" + turingElement.getId());
			}else {
		...
		paymentAccounts.forEach(paymentAccountEntity -> {
			String tokenizedBankAccountNumber = paymentAccountEntity.getPmtAcctUnqId().toString();
			...
			if (map.containsKey(tokenizedBankAccountNumber)) {
				...
				logger.warn("Skipping the record while updating pams as invalid data is found in the record with paymentAccountUniqueId ["+ paymentAccountEntity.getPmtAcctUnqId()+"].");
			}
		});
